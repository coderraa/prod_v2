...
async function coolercomplain(a,body) {
  try {
    const response = await axios.get('https://800mycoke.ae/askArwa/getChatResponse.jsp?customerid='+a);
    console.log(response.status);
    const responsemail = await axios.get('https://800mycoke.ae/askArwa/sendMail.jsp?customerId='+a+'&body='+body+'&countryCode='+response.data.country);
...
async function verify(a,id1,id2) {
  try {
    const response = await axios.get('https://800mycoke.ae/askArwa/verifyEquipment.jsp?customerId='+a+'&assetId='+id1+'&assetId2='+id2+'&msg='+"not varified");
    console.log(response.status);
